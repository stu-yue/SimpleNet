/**
 * @file app_server.c
 * @brief 	这是服务器应用程序代码. 服务器首先通过在客户端和服务器之间创建TCP连接,启动重叠网络层. 
 * 			然后它调用stcp_server_init()初始化STCP服务器. 
 * 			它通过两次调用stcp_server_sock()和stcp_server_accept()创建2个套接字并等待来自客户端的连接. 
 * 			最后, 服务器通过调用stcp_server_close()关闭套接字.
