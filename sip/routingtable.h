...
/**
 * @brief   这个函数动态创建路由表.表中的所有条目都被初始化为NULL指针.
 *          然后对有直接链路的邻居,使用邻居本身作为下一跳节点创建路由条目,并插入到路由表中.
 *          该函数返回动态创建的路由表结构.
 * 
 * @return routingtable_t* 
 */
...
/**
 * @brief   这个函数使用给定的目的节点ID和下一跳节点ID更新路由表.
...
/**
 * @brief   这个函数在路由表中查找指定的目标节点ID.
 *          为找到一个目的节点的路由条目, 你应该首先使用哈希函数makehash()获得槽号,
 *          然后遍历该槽中的链表以搜索路由条目.如果发现destNodeID, 
 *          就返回针对这个目的节点的下一跳节点ID, 否则返回-1.
 * 
 * @param routingtable 
 * @param destNodeID 
 * @return int 
